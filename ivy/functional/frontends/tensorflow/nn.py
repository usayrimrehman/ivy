...
from ivy.functional.frontends.tensorflow import general_functions as gen_func
...
#   new_seq_len = math.reduce_sum(cast(label_mask,ivy.int32), axis=1)._ivy_array
  ...
  idx = gen_func.range(ivy.shape(flat_idx_mask)[0])._ivy_array #!!!!
  ...
  for i in range(len(indices)):
    flat[indices[i]]+=updates[i]

  # Reshape back to square batch.new_seq_len
  batch_size = ivy.shape(labels)[0] #!!!
  new_shape = [int(batch_size), int(new_maxlen)]
  return (ivy.reshape(flat, new_shape),new_seq_len)